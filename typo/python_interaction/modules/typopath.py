import os
import genericpath, posixpath
from ._core import _get
...
def normcase(path): return path
def normpath(path):
    partway = path.replace('\\', '/')
    while '//' in partway:
        partway = partway.replace('//', '/')
    return partway
...
def abspath(path):
    if not isabs(path):
        fsroot = _get('fsroot')
        lower = path.lower()
        absolute = os.path.abspath(os.path.join(fsroot, lower))
        absolute_orig = os.path.abspath(os.path.join(fsroot, path))
        sub = os.path.commonpath([fsroot, absolute])
        winabs = absolute_orig[len(sub):]
        typabs = normpath(winabs)
        return typabs.replace('/', '::', 1)
    else: return path